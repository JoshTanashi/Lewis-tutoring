...
export default async function StudentDashboard() {
  ...
  const { data: me } = await supabase
    .from("students")
    .select("id, full_name, mascot, confidence_score, grade")
  ...
  const [{ data: nextLesson }, { data: homework }, { data: badges }, { data: streak }] =
    await Promise.all([
      supabase
        .from("lessons")
        .select("scheduled_at, mode, subjects(name, emoji)")
        .eq("student_id", me.id)
        .eq("status", "scheduled")
  ...
  return (
    <div className="space-y-6">
      {/* welcome banner */}
      <Card sticker className="relative overflow-hidden bg-pastel-yellow p-6">
        <RainbowArc size={160} className="absolute -right-6 -top-8 opacity-60" />
        <div className="flex items-center gap-4">
          <div className="animate-bounce-soft">{mascotEl(me.mascot)}</div>
          <div>
            <h1 className="font-display font-bold text-3xl">Hi {firstName}! ⭐</h1>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Card sticker className="bg-pastel-pink p-4">
        ...
        <StatCard sticker label="Streak" value={`${streak?.current_weeks ?? 0} weeks`} emoji="🔥" tone="sunshine" hint="lessons in a row!" />
        <StatCard sticker label="Badges" value={badges?.length ?? 0} emoji="🏅" tone="lilac" hint="on your shelf" />
        <Card sticker className="bg-pastel-green p-4">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card sticker className="p-5">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="font-display font-bold text-xl">Today&apos;s missions 🚀</h2>
